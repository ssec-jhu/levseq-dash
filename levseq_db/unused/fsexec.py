#
# fsexec.py  -- TODO: UNUSED IF USING LEVSEQ WEBSERVICE
#
# Notes:
...
def UploadExperimentFile(uid: int, gid: int, fileSpec: str, fileContents: str) -> int:

    # the LevSeq files should rarely be more than a few MB in size, so we just ship
    #  the filename and contents to the webservice and assume that the usual HTTP
    #  request mechanism can handle a multi-megabyte "parameter"
    return dbexec.QueryScalar(
        "upload_experiment_file", [uid, gid, fileSpec, fileContents]
    )  # type:ignore
    ...
    if False:
        ...
        b64bytes = fileContents.encode("utf8").split(b";base64,")[1]
